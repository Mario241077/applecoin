...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MAC unit input in lnReqAmount, we generate the URI with MAC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MAC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
